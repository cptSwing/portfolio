...
export type SelectProps = Omit<ThreeElements['group'], 'ref'> & {
    ...
    border?: string;
    /** Custom CSS color, default: 'rgba(75, 160, 255, 0.1)' */
...
export function SelectInstances({
    ...
    const onClick = React.useCallback((e: ThreeEvent<MouseEvent>) => {
        e.stopPropagation();
        console.log('%c[SelectInstances]', 'color: #d5184a', `e.instanceId :`, e.instanceId);
        dispatch({ object: customFilter([e.object])[0], shift: multiple && e.shiftKey });
    }, []);

    const onPointerMissed = React.useCallback((_e: MouseEvent) => !hovered && dispatch({}), [hovered]);

    const ref = React.useRef<THREE.Group>(null!);

    // Only for box selections / multiple:
    React.useEffect(() => {
        ...
        function prepareRay(event: MouseEvent, vec: THREE.Vector3) {
            const { offsetX, offsetY } = event;
            const { width, height } = size;
            vec.setX((offsetX / width) * 2 - 1);
            vec.setY(-(offsetY / height) * 2 + 1);
        }
        ...
        function pointerUp(_event: MouseEvent) {
    ...
    return (
        <group ref={ref} onClick={onClick} onPointerOver={() => hover(true)} onPointerOut={() => hover(false)} onPointerMissed={onPointerMissed} {...props}>
